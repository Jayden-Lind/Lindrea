...
def get_listing(listing):
    ...
    listing_company_address = listing_company.get("address", {}).get("display", {}).get("fullAddress")
    ...
    floorplan = ""
    if len(listing.get('media', {}).get('floorplans', {})) > 0:
        size_regex = r'\{size\}'
        size_regex_replace = "1000x1000-resize"
        floorplan_template_url = listing.get('media', {}).get('floorplans', {})[0].get('templatedUrl')
        floorplan = re.sub(size_regex,  size_regex_replace, floorplan_template_url)
    ...
    listing_company = {
        ...
        'listing_company_address' : listing_company_address
    }
